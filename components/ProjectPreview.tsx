...
import projectPic from "../public/clinic-portal.jpg";
...
const ProjectPreview: React.FC<Props> = ({
  ...
  return (
    <motion.div
      className={`md:min-h-[20rem] rounded-3xl overflow-hidden ${colSpan} group`}
      style={style}
      {...motionProps}
    >
      <div
      ...
      >
        ...
        <div className="w-5/6 max-w-lg my-4 z-10">
          <p
            className={`text-gray-700 dark:text-slate-light/80 ${
          ...
          {description.length > 120 && (
            <button
              ...
              className="text-sm text-gray-400 dark:text-slate-muted/70 hover:text-gray-700 dark:hover:text-slate-light mt-1 underline underline-offset-2"
            >
        ...
        <div className="flex flex-wrap justify-items-start gap-x-4 gap-y-2 z-10">
          {badges.map((badge) => (
            <div
              key={badge}
              className="h-6 px-3 bg-white rounded-full text-sm flex items-center justify-center text-gray-400 dark:bg-navy-700 dark:text-slate-light"
            >
              {badge}
            </div>
          ))}
